    ...
    return async inputEvent => {
      if (inputEvent.keyCode === 13) {
        await $this.model.addTask(
          { name: inputEvent.target.value, state: false },
          parent
        );
  ...
  folderRender(folder) {
    ...
    folder.forEach((v, k) => {
      ul.appendChild(
        create("li", {
          html: v.name,
          event: { click: this.taskRender(v.name, k) }
        })
      );
    });
